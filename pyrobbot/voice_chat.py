...
class VoiceChat(Chat):
    ...
    def handle_speak_queue(self, wav_buffer_queue: queue.Queue[io.BytesIO]):
        """Handle the queue of WAV buffers to be played."""
        while True:
            try:
                self.speak(wav_buffer_queue.get())
